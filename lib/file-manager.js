...
class FileManager {
   ...
   * @returns {promise} Promise with response object
   */
  static async createFolder(folderName, acl) {
    if (!folderName.match(/\/$/)) { return Promise.reject({ status: 400, message: `Invalid folder name: ${folderName}`}); }
    return Promise.resolve(await FileManager.write(folderName, undefined, acl));
  }
   ...
   * @returns {promise} Promise with response object
   */
  static async createFile(fileName, buffer, acl) {
    if (fileName.match(/\/$/)) { return Promise.reject({ status: 400, message: `Invalid file name: ${fileName}`}); }
    return Promise.resolve(await FileManager.write(fileName, buffer, acl));
  }
