...
class FileManager {
  ...
  /**
   * Get S3 object and convert the buffer to string
   * 
   * @param {string} objectKey Object key. Either folder or file object key
   * @returns {promise} Promise with s3 object string
   */
  static async readToString(objectKey) {
    return Promise.resolve(await (await FileManager.read(objectKey)).Body.transformToString());
  }

  /**
   * Get S3 object and convert the buffer to base64 string
   * 
   * @param {string} objectKey Object key. Either folder or file object key
   * @returns {promise} Promise with s3 object string
   */
  static async readToBase64(objectKey) {
    return Promise.resolve((await FileManager.readToBuffer(objectKey)).toString("base64"));
  }

  /**
   * Get S3 object into buffer
   * 
   * @param {string} objectKey Object key. Either folder or file object key
   * @returns {promise} Promise with s3 object buffer
   */
  static async readToBuffer(objectKey) {
    return Promise.resolve(Buffer.from(await (await FileManager.read(objectKey)).Body.transformToByteArray()));
  }
